...
import { referendumStatusV2 } from '../util/statuses';
import {
  Cached,
...
const l = logger('Task: FellowshipReferendum Status Update');
 ...
 */
const createFellowshipReferendumStatusV2: Task<NomidotReferendumV2StatusUpdate[]> = {
  name: 'createFellowshipReferendumStatusUpdate',
  read: async (
    _blockHash: Hash,
    cached: Cached,
    _api: ApiPromise
    ...
    await Promise.all(
      referendumEvents.map(async ({ event: { data, typeDef, method } }) => {
        ...
        l.log(`fellowshipreferendumRawEvent: ${JSON.stringify(referendumRawEvent)}`);
        ...
        const relatedReferendum = await prisma.fellowshipReferendum({
        ...
        if (referendumRawEvent.Tally){
          ...
          tallyData.bareAyes = referendumRawEvent.Tally.bareAyes ? hexToNumber(referendumRawEvent.Tally.bareAyes).toString() : "0";
    ...
    if (value && value.length) {
      await Promise.all(
        value.map(async ref => {
          ...
          await prisma.createFellowshipReferendumStatus({
            blockNumber: {
              connect: {
          ...
          if (tally) {
            ...
            if (relatedReferendum){
              await prisma.updateFellowshipReferendum({
                where: {
...
export default createFellowshipReferendumStatusV2;
