...
const l = logger('Task: FellowshipReferenda');
...
const createFellowshipReferendum: Task<NomidotReferendumV2[]> = {
  name: 'createFellowshipReferendum',
  read: async (
    blockHash: Hash,
    cached: Cached,
    api: ApiPromise
    ...
    await Promise.all(
      referendumEvents.map(async ({ event: { data, typeDef } }) => {
        ...
        if (
          ...
          l.error(
            `Expected Fellowship ReferendumIndex missing in the event: ${referendumRawEvent.ReferendumIndex}`
          );
          return null;
        }
  ...
  write: async (blockNumber: BlockNumber, value: NomidotReferendumV2[]) => {
    ...
    await Promise.all(
      value.map(async referendum => {
        ...
        await prisma.createFellowshipReferendum({
...
export default createFellowshipReferendum;